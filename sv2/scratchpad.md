...
## v4: Fix Reward/Env Logic

### The "Rewarding" Bug
**Symptom**: User noted "Env is shoddy" and "Reward Model does not look right".
**Root Cause**: 
1. `code_verify_interaction.py` was returning `0.0` reward because `calculate_score` was a placeholder.
2. `Sv2RewardManager` (in `reward.py`) was **overwriting** any potential interaction rewards by re-calculating the score based only on the final `#### answer`. It ignored the `turn_scores` generated by the interaction.

**Fixes Applied**:
1. **Interaction**: Updated `CodeVerifyInteraction.calculate_score` to check if the generated code contains the `ground_truth` number. If yes, returns `+0.5`.
2. **Reward Manager**: Updated `Sv2RewardManager` to check for `turn_scores` in the `DataProto`. It now sums these interaction rewards and adds them to the final outcome reward (`final_reward = outcome_reward + sum(turn_scores)`).

### Modular Structure Verification
The structure **works**:
- **Inference (`sv2/eval.py`)**: Correctly uses `ToolAgentLoop` to generate multi-turn rollouts.
- **Env (`sv2/interactions/`)**: Now correctly computes logic-based rewards during the interaction loop.
- **Reward (`sv2/reward.py`)**: Now acts as a proper aggregator of "Outcome Reward" (GSM8K match) and "Process/Interaction Reward" (Code verification).
- **Training (`sv2/train.py`)**: Structure is valid. It correctly delegates rollout generation and reward computation. The PPO update itself remains a placeholder, as intended.

### Next Steps
- Implement actual PPO update in `sv2/train.py` using `verl.trainer.ppo.core_algos`.

---

## v5: Tools & PPO Logic

### 1. Parquet vs Direct Loading
- **Why Parquet?**: verl is designed for distributed RLHF where data efficiency is key. `DataProto` expects pre-processed, columnar data.
- **Can we load directly?**: Yes, but requires writing a custom `DataFlow` or modifying pre-processing to happen on-the-fly in memory. The current flow (`preprocess -> parquet -> train`) is the "happy path" for verl.

### 2. Interaction in Code
- `ToolAgentLoop` loads interactions via `initialize_interactions_from_config` (YAML).
- **To do it in code**:
  - You can manually register/patch `ToolAgentLoop.interaction_map` before running the loop.
  - Or create a dictionary of interactions and subclass `ToolAgentLoop` to accept them.
  - Current workaround: Created `sv2/config/interaction_config.yaml` as the standard way, but `ToolAgentLoop` registry is just a dict you can modify at runtime if needed.

### 3. Python Exec Tool
- Created `sv2/tools/python_tool.py`:
  - Implements `PythonExecTool` using `exec()`.
  - **WARNING**: Not secure for untrusted code.
- Created `sv2/config/tool_config.yaml`:
  - Registers `python_exec` tool.
  - To use: add `actor_rollout_ref.rollout.multi_turn.tool_config_path=sv2/config/tool_config.yaml` to the run command.

### 4. PPO Logic in `train.py`
- **Implemented**: Real PPO Training Logic
  - Instantiates `Actor`, `Ref`, and `Critic` models in `train.py`.
  - Implements `_ppo_update` with full GAE + PPO Clipping + Value Loss.
  - Updated `run_training_loop` to initialize these components.
- **Config**: Updated `sv2/config/sv2_multiturn.yaml` to include `critic`, `optim`, and `algorithm` params.

### 5. Tool Parallelism
- **Implemented**: Updated `PythonExecTool` to run `exec()` in `loop.run_in_executor` (thread pool) to allow non-blocking async execution.